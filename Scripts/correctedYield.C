...
int main(int argc, char* argv[]) {
  ...
  auto histEventCuts =
      FileReader::GetHist1D(filenameData, appendixData, "EvtCuts",
                            {{"EvtCuts", "Event Cuts"}}, "EventCounter");
  const float nEvents = histEventCuts->GetBinContent(10);

  // Histogram for Sigma0 Integral width
  auto histSigmaCuts =
      FileReader::GetHist1D(filenameData, appendixData, "Sigma0Cuts",
                            {{"Sigma0Cuts"}}, "fHistCutBooking");
  const float intervalWidth = histSigmaCuts->GetBinContent(1);

  // Sigma0 inv. mass spectrum from data
  auto sigmaHistData =
      FileReader::GetHist2D(filenameData, appendixData, "Sigma0Cuts",
                            {{"Sigma0Cuts"}}, "fHistInvMassPt");
  auto antiSigmaHistData =
      FileReader::GetHist2D(filenameData, appendixData, "AntiSigmaCuts",
                            {{"AntiSigmaCuts"}}, "fHistInvMassPt");

  // Sigma0 inv. mass spectrum from MC
  //  auto sigmaHistMC = FileReader::GetHist2D(filenameMC, appendixMC,
  //  {{"Sigma0"}},
  //                                           "fHistInvMassPt");
